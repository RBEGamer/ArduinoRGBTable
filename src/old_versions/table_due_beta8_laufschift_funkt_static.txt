...
//LAUFSCHRIFT VARS ----------------------------------------------------------------------------------
int st_text_pos = 0;
const int st_maxchar = 50;
char st_chars[st_maxchar] = {'h','e','l','l','o',' ','w','o','r','l','d'};
int st_char_count = 11;
const int st_spacing = 2;
int st_laufschrift_complete_arr[((5+st_spacing)*st_maxchar)+feldx][7];
int st_anchor[2] = {0,0};
int st_speed = 50;
...
st_laufschrift_complete_arr[(k*st_spacing*3)+i][j] = char_template[i][j];
}}
...
st_text_to_arr();
  ...
  if(st_text_pos >= ((5+st_spacing)*st_char_count)+feldx){
  st_text_pos = 0;
  }else{
  st_text_pos++;
  }
 ...
 int st_color =  Wheel(st_color_counter);
...
if(st_laufschrift_complete_arr[st_text_pos+i-20][j] == 1){
  ...
  if (type == WebServer::POST)
  {
    ...
    do
    {
      ...
      if (strcmp(name, "ransp") == 0){randomspeed = strtoul(value, NULL, 10);}
          ...
          if (strcmp(name, "stpos") == 0){st_text_pos = strtoul(value, NULL, 10);}
      if (strcmp(name, "stspeed") == 0){st_speed = strtoul(value, NULL, 10);}
...
server<<"<td scope='col'><input type='radio' name='mode' id='mode'  value='8' ";if(mode == 8){server <<"checked";}server<<">SCROLLING TEXT</td>";
server<<"<td><input type='submit' value='SAVE' /></td> ";
server<<"</tr>  ";
server<<"</table>";
server<<"</form>";
...
server<<"<table width='50%' border='0'>";
server<<"<tr>";
server<<"<td scope='col'> <label for='stpos'>stpos</label></td>";
server<<"<td>";
server<<"    SPEED : <br>";
server<<"</td> ";
server<<"<td scope='col' >";
server<<"<input name='stpos' type='number' id='stpos' max='";server<<(5+st_spacing)*st_char_count+feldx; server<<"' min='0' step='1' value='"; server<<st_text_pos;server <<"' size='20'><br>";
server<<"</td>  ";
server<<"<td colspan='2'><input type='submit' value='SAVE' /></td>";
server<<"</tr>";
server<<"</table> ";
...
server<<"</td> ";
server<<"<td scope='col' >";
server<<"<input name='stpos' type='number' id='stspeed' max='1000' min='0' step='1' value='"; server<<st_speed;server <<"' size='20'><br>";
server<<"</td>  ";
server<<"<td colspan='2'><input type='submit' value='SAVE' /></td>";
server<<"</tr>";
server<<"</table> ";
...
void main_process(){
 ...
 getDateDs1307();
 if(fan_state == 1 && mode != 5){digitalWrite(fan,HIGH);}else{digitalWrite(fan,LOW);}
}
